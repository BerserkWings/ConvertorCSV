""" VERSION 1: Obtiene los siguientes datos: edit, ip-address, subnet-mask, color, comments. NO obtiene el fqdn y unicamente obtiene todos los datos que tengan una mascara 255.255.255.255
...
def cargar_archivo():
    ...
    if archivo_path: # Si se selecciono un archivo, es decir, si esta variable tiene un valor, entonces
...
def opciones_Realizar():
    ...
    # Si los cualquiera de los widgets no tiene contenido, es decir, si no hay ruta de archivo cargado o no hay ruta de directorio de guardado, entonces:
    ...
    if resultado_path: # Si este widget tiene un valor, entonces
        ...
        procesar_archivo(txt_nombre_archivo.get(1.0, tk.END).strip(), opciones_seleccionadas, resultado_path) # Se llama a la función procesar archivo, este usara los sig. parametros: ruta completa del archivo cargado (por eso el .strip), la variable opciones_seleccionadas y ruta con el nombre del archivo a guardar)
...
# Función para borrar las opciones usadas
...
def procesar_archivo(archivo_path, opciones_seleccionadas, resultado_path):
    with open(archivo_path, 'r') as archivo:
        datos = archivo.read()

    # Dividir datos por conjuntos (asumiendo que cada conjunto comienza con 'edit')
    ...
    for conjunto in conjuntos:
        ...
        if "edit" in opciones_seleccionadas:
            match_edit = re.search(r'edit "(.*?)"', conjunto)
            if match_edit:
                conjunto_dict["edit"] = match_edit.group(1)

        if "comments" in opciones_seleccionadas:
            match_comments = re.search(r'set comment "(.*?)"', conjunto)
            if match_comments:
                conjunto_dict["comments"] = match_comments.group(1)

        if "ip-address" in opciones_seleccionadas:
            match_subnet = re.search(r'set subnet (\d+\.\d+\.\d+\.\d+) \d+\.\d+\.\d+\.\d+', conjunto)
            if match_subnet:
                conjunto_dict["ip-address"] = match_subnet.group(1)

        if "subnet-mask" in opciones_seleccionadas:
            # Asumiendo que siempre es 255.255.255.255
            conjunto_dict["subnet-mask"] = "255.255.255.255"

        if "color" in opciones_seleccionadas:
            conjunto_dict["color"] = "black"

        if "fqdn" in opciones_seleccionadas:
            match_fqdn = re.search(r'set fqdn "(.*?)"', conjunto)
            if match_fqdn:
                conjunto_dict["fqdn"] = match_fqdn.group(1)

        # Verificar que todos los campos requeridos estén presentes
        if all(opcion in conjunto_dict for opcion in opciones_seleccionadas):
            datos_procesados.append(conjunto_dict)

    guardar_en_csv(datos_procesados, resultado_path, opciones_seleccionadas)

def guardar_en_csv(datos, archivo_csv, opciones_seleccionadas):
    with open(archivo_csv, 'w', newline='') as archivo:
        escritor_csv = csv.writer(archivo)
        escritor_csv.writerow(opciones_seleccionadas)

        for conjunto in datos:
            fila = [conjunto.get(opcion, '') for opcion in opciones_seleccionadas]
            escritor_csv.writerow(fila)

def _dict_to_list(diccionario, orden):
    return [diccionario.get(clave, '') for clave in orden]

if __name__ == "__main__":
    ventana = tk.Tk()
    ventana.title("LectorCVS")
    ...
    mb_opciones = tk.Menubutton(ventana, text="Opciones a usar")
    mb_opciones.pack(pady=10, padx=20)

    submenu_opciones = tk.Menu(mb_opciones, tearoff=0)
    submenu_opciones.add_command(label="edit", command=lambda: actualizar_opciones_usadas("edit"))
    ...
    btn_iniciar = tk.Button(ventana, text="Generar CSV's", command=opciones_Realizar)
    ...
    ventana.mainloop()
"""
""" VERSION 2: Esta versión obtiene unicamente los FQDN, opciones que sirven: edit, fqdn, color, comments
import tkinter as tk
from tkinter import filedialog, simpledialog, messagebox
import csv
import re
...
def procesar_archivo(archivo_path, opciones_seleccionadas, resultado_path):
    ...
    for i, conjunto in enumerate(conjuntos):
        ...
        if "fqdn" in opciones_seleccionadas:
            match_type_fqdn = re.search(r'set type fqdn', conjunto)
            if match_type_fqdn:
                match_fqdn = re.search(r'set fqdn "(.*?)"', conjunto)
                if match_fqdn:
                    conjunto_dict["fqdn"] = match_fqdn.group(1)
                else:
                    print(f"No se encontró fqdn en el conjunto {i + 1}: {conjunto}")
...
if __name__ == "__main__":
    ...
    ventana.mainloop()
"""
"""VERSION 3: Este obtiene todo lo que quieras, pero no tiene excepciones ni el boton opciones a mostrar.
import tkinter as tk
from tkinter import filedialog, simpledialog, messagebox
import csv
import re
...
def opciones_Realizar():
    ...
    if resultado_path: # Si este widget tiene un valor, entonces
        ...
        # Limpiar el texto de las etiquetas
        ...
        txt_nombre_archivo.see(tk.END)
        txt_ruta_guardado.see(tk.END)
        txt_opciones_usadas.see(tk.END)

        # Ajusta la vista al final del texto
        txt_nombre_archivo.update_idletasks()  # Actualizar antes de ver
        txt_ruta_guardado.update_idletasks()  # Actualizar antes de ver
        txt_opciones_usadas.update_idletasks()  # Actualizar antes de ver
...
def procesar_archivo(archivo_path, opciones_seleccionadas, resultado_path):
    ...
    for i, conjunto in enumerate(conjuntos):
        ...
        if "subnet-mask(variado)" in opciones_seleccionadas:
            match_subnet_variado = re.search(r'set subnet (\d+\.\d+\.\d+\.\d+) (\d+\.\d+\.\d+\.\d+)', conjunto)
            if match_subnet_variado:
                subnet_mask_variada = match_subnet_variado.group(2)
                if subnet_mask_variada != "255.255.255.255":
                    conjunto_dict["subnet-mask(variado)"] = subnet_mask_variada
...
def guardar_en_csv(datos, archivo_csv, opciones_seleccionadas):
    with open(archivo_csv, 'w', newline='') as archivo:
        ...
        opciones_seleccionadas_cabecera = [opcion.replace("(variado)", "") for opcion in opciones_seleccionadas]
        escritor_csv.writerow(opciones_seleccionadas_cabecera)

        for conjunto in datos:
            fila = [conjunto[opcion] for opcion in opciones_seleccionadas]
            escritor_csv.writerow(fila)
...
if __name__ == "__main__":
    ...
    ventana.mainloop()
"""
""" VERSION 4: Aqui ya tiene el boton y las opciones de mostrar datos, pero aun no funciona el filtrado para que muestre lo que desea el usuario
import tkinter as tk
from tkinter import filedialog, simpledialog, messagebox
import csv
import re
...
def actualizar_mostrar_datos(opcion): # Actualiza el contenido del widget txt_opciones_usadas y muestra las opciones usadas en la etiqueta opciones_usadas
    ...
    opciones_nuevas = f"{datos_actuales}, {opcion}" if datos_actuales else opcion # Construye nueva cadena, usando el contenido de la variable opciones_actuales y le agrega la siguiente opción que el usuario escoja, separandolas por comas y un espacio
    ...
    txt_mostrar_datos.insert(tk.END, opciones_nuevas) # Insertar nuevas opciones
...
if __name__ == "__main__":
    ...
    submenu_mostrar.add_command(label="subnet-mask(variado)", command=lambda: actualizar_mostrar_datos("subnet-mask(variado)"))
    ...
    ventana.mainloop()
"""
"""VERSION 5: Esta es la versión semi final, solamente falta mejorar un poco la interfaz, manejar posibles errores y 
import tkinter as tk
from tkinter import filedialog, simpledialog, messagebox
import csv
import re
...
def opciones_Realizar():
    ...
    if resultado_path: # Si este widget tiene un valor, entonces
        ...
        resultado_path = resultado_path  # Se asegura de que el archivo tenga la extensión .csv
        procesar_archivo(txt_nombre_archivo.get(1.0, tk.END).strip(), opciones_seleccionadas, resultado_path, opciones_mostrar) # Se llama a la función procesar archivo, este usara los sig. parametros: ruta completa del archivo cargado (por eso el .strip), la variable opciones_seleccionadas y ruta con el nombre del archivo a guardar)
        ...
        txt_ruta_guardado.see(tk.END)
        txt_opciones_usadas.see(tk.END)
        txt_mostrar_datos.see(tk.END)

        # Ajusta la vista al final del texto
        txt_nombre_archivo.update_idletasks()  # Actualizar antes de ver
        txt_ruta_guardado.update_idletasks()  # Actualizar antes de ver
        txt_opciones_usadas.update_idletasks()  # Actualizar antes de ver
        txt_mostrar_datos.update_idletasks()
...
def procesar_archivo(archivo_path, opciones_seleccionadas, resultado_path, opciones_mostrar):
    ...
    guardar_en_csv(datos_procesados, resultado_path, opciones_mostrar)
...
def guardar_en_csv(datos, archivo_csv, opciones_mostrar):
    with open(archivo_csv, 'w', newline='') as archivo:
        ...
        escritor_csv.writerow(opciones_mostrar_cabecera)

        for conjunto in datos:
            conjunto_filtrado = {opcion: conjunto[opcion] for opcion in opciones_mostrar if opcion in conjunto}
            fila = [conjunto_filtrado.get(opcion, '') for opcion in opciones_mostrar_cabecera]
            escritor_csv.writerow(fila)
    ...
    try:
        ...
        if resultado_path: # Si este widget tiene un valor, entonces
            ...
            if "fqdn" in opciones_seleccionadas:
                ...
                opciones_invalidas = set(["ip-address", "subnet-mask", "subnet-mask(variado)"]) & set(opciones_seleccionadas)
...
def procesar_archivo(archivo_path, opciones_seleccionadas, resultado_path, opciones_mostrar):
    ...
    for i, conjunto in enumerate(conjuntos):
        ...
        if "subnet-mask(variado)" in opciones_seleccionadas:
            ...
            if match_subnet_variado: # Si existe
